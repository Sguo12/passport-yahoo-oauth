...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  ...
  this._oauth.get('https://social.yahooapis.com/v1/user/' + params.xoauth_yahoo_guid + '/profile?format=json', token, tokenSecret, function (err, body, res) {
    ...
    try {
      ...
      that._oauth.get('https://social.yahooapis.com/v1/user/' + params.xoauth_yahoo_guid + '/contacts?format=json', token, tokenSecret, function (err, body, res) {
          if (err) { return done(err); }

          try {
              profile.contacts = JSON.parse(body);      
              done(null, profile);
          }
          catch (e) {
              done(e);
